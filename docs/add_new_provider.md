...
There are **two patterns** used in Cline today:

- **Pattern A – Generic provider + model ID (recommended, used by Gemini, Vertex, Anthropic, DeepSeek, Qwen, etc.)**
  - Uses shared fields `planModeApiProvider` / `actModeApiProvider` and `planModeApiModelId` / `actModeApiModelId`.
  - You only add provider-specific secrets and extras (for example, `geminiApiKey`, `geminiBaseUrl`, `geminiPlanModeThinkingLevel` / `geminiActModeThinkingLevel`).

- **Pattern B – Provider-specific model fields (used by OpenRouter, Requesty, Aihubmix, etc.)**
  - Uses custom fields such as `planModeOpenRouterModelId`, `actModeOpenRouterModelId`, often with associated `...ModelInfo`.
  - Recommended only when the provider needs richer per-model metadata or a non-standard selection UX.

When in doubt, follow **Pattern A** and use **Gemini** as your reference implementation.
...
export function normalizeApiConfiguration(
  ...
  const getProviderData = (models: Record<string, ModelInfo>, defaultId: string) => {
    const id = modelId && modelId in models ? modelId : defaultId
    return {
      selectedProvider: provider,
      selectedModelId: id,
      selectedModelInfo: models[id],
    }
  }

  switch (provider) {
    // Pattern A: Providers that use generic planModeApiModelId/actModeApiModelId (Gemini, Vertex, Anthropic, etc.)
    case "gemini":
      return getProviderData(geminiModels, geminiDefaultModelId)

    // Pattern B: Providers with custom model fields (OpenRouter, Requesty, Aihubmix, etc.)
    case "your-provider": {
